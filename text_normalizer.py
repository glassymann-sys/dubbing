# -*- coding: utf-8 -*-
"""
Uzbek TTS — Text Normalizer v4
Исправлены: апострофы, HTTPS→havola, SMS дефис, XY_456, дата
"""

import re

# ─────────────────────────────────────────────
# Апостроф в узбекском (U+2019 правая кавычка)
# Edge TTS принимает оба варианта, используем обычный ASCII '
# ─────────────────────────────────────────────
APO = "'"   # ASCII апостроф — безопасен везде
...
def digits_one_by_one(digits: str) -> str:
    """998901 → to'qqiz to'qqiz sakkiz nol bir"""
    return " ".join(num(int(d)) for d in digits if d.isdigit())
...
ABBR = {
    "HTTPS": "ey-ti-ti-pi-es",   # должен быть ДО HTTPS→havola
    "HTTP":  "ey-ti-ti-pi",
    "UZS":   f"so{APO}m",
    "USD":   "dollar",
    "EUR":   "evro",
...
def normalize(text: str) -> str:
    ...
    text = re.sub(r'\b(\d{1,2})\.(\d{2})\.(\d{4})\b', date_fmt, text)
    ...
    # 8. Аббревиатуры (SMS → es-em-es и т.д.)
    text = expand_abbr(text)

    # 9. Подчёркивание → пробел (ДО mixed_codes, чтобы XY_456 → XY 456)
    text = text.replace('_', ' ')

    # 10. Смешанные коды: BV3, XY456, ABC → по буквам
    def mixed(m):
        tok = m.group(0)
    ...
    # 11. Большие числа с пробелами: 1 000 000
    text = re.sub(
        r'\b\d{1,3}(?:\s\d{3})+\b',
    ...
    text = re.sub(r'[(){}\[\]]',  ', ',  text)
    # ВАЖНО: удаляем только «типографские» кавычки, не ASCII апостроф!
    text = re.sub(r'[«»""\u2018\u201C\u201D]', '', text)
    text = re.sub(r'[@$^&*+=|\\<>~`]', ' ', text)
...
if __name__ == "__main__":
    tests = [
        ...
        ("Bugun 15.06.2025 da 1000000 so'm to'landi",
         "o'n besh-iyun, ikki ming yigirma besh-yil ... bir million som ..."),

        ("Mijoz ID #4521 — VIP status — 30% chegirma",
         "Mijoz aydiy raqam to'rt ming ... vi-ay-pi ... o'ttiz foiz chegirma"),
    ]

    print("=" * 65)
    print("🧪 TEXT NORMALIZER v4")
    print("=" * 65)
